...
class MCAPLiDARLoader:
    """Reads PointCloud2 messages from ROS2 MCAP bags using rosbags AnyReader."""
    
    def __init__(self, target_path: str, topic_name: str = "/livox/lidar"):
        path = Path(target_path)
        if path.is_dir():
            mcap_files = list(path.glob("*.mcap"))
            if not mcap_files:
                raise FileNotFoundError(f"No .mcap file found inside directory: {path}")
    ...
    def stream_point_clouds(self) -> Generator[Tuple[float, np.ndarray, np.ndarray], None, None]:
        """Streams (timestamp_sec, xyz_points, intensity) tuples."""
        with AnyReader([self.mcap_path], default_typestore=self.typestore) as reader:
            ...
            if not connections:
                raise ValueError(f"No PointCloud2 topics found in {self.mcap_path}")
    ...
    def _unpack_point_cloud2(msg) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        try:
            ...
            num_points = msg.width * msg.height if hasattr(msg, "width") and msg.width > 0 else len(data) // point_step
            ...
            if offsets["intensity"] + 4 <= point_step:
                ...
                intensity = np.ones_like(x) * 100.0
